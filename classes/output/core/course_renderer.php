        ...
        if (count($rcourseids) > 0) {
            foreach ($acourseids as $courseids) {
                ...
                foreach ($courseids as $courseid) {
                    ...
                    $summary = theme_boost_union_strip_html_tags($course->summary);
                    $summary = format_text(theme_boost_union_course_trim_char($summary, $trimsummaryvalue));
                    $trimtitle = format_string(theme_boost_union_course_trim_char($course->fullname, $trimtitlevalue));
                    ...
                    if ($coursetilestyle == 2) {
                        ...
                        $enrollbutton = get_string('enrollcoursecard', 'theme_boost_union');
                        $rowcontent .= '
                <div class="col-md-4">
                    ';
                        $rowcontent .= '
...
background-blend-mode: overlay;">
    ...
    public function frontpage_my_courses() {
        ...
        $nomycourses = '<div class="alert alert-info alert-block">' . get_string('nomycourses', 'theme_boost_union') . '</div>';
        $lastaccess = '';
        $output = '';
        $showbylastaccess = 1;
